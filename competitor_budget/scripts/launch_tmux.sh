...
MODE="${MODE:-observe}"
[[ "$MODE" == observe || "$MODE" == compare ]] || { echo 'MODE must be observe or compare'; exit 2; }
...
for name in LIMIT MAX_NEW_TOKENS BLOCK_SIZE SMALL_BLOCK_SIZE THRESHOLD MARGIN USE_BLOCK_CACHE CONDA_ENV CONDA_ROOT MIN_FREE_GIB HF_HOME HF_HUB_CACHE HF_DATASETS_CACHE; do
    if [[ -v "$name" ]]; then
        printf 'export %s=%q\n' "$name" "${!name}" >> "$RUN_DIR/job.env"
    fi
done
