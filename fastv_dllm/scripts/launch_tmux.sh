...
mkdir -p "$RUN_DIR"
for name in REPO MODE RUN_DIR GPU_IDS DATASET TASK LIMIT GEN_LENGTH BLOCK_LENGTH THRESHOLD PRUNE_AFTER_LAYER SUPPORT_KEEP_RATIO CONTEXT_KEEP_RATIO ANCHOR_PREFIX RECENT_CONTEXT METHODS REQUIRE_IDLE CONDA_ROOT CONDA_ENV HF_HOME HF_HUB_CACHE HF_DATASETS_CACHE; do
