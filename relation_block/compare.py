...
def main():
    ...
    for key in ("ids", "max_new_tokens", "data_hash", "block_size", "gpu", "torch"):
    ...
    for key in ("steps", "completed_steps", "original_tokens", "global_batch", "lr", "rank", "seed", "objective", "adaptation", "implementation"):
        if trained and any(x[key] != trained[0][key] for x in trained):
            raise ValueError(f"Training mismatch: {key}")
    print("arm\trounds\taccuracy\tseconds\ttokens/s\tcalls")
    for x in rows:
        for rounds, r in x["results"].items():
            print(f"{x['arm']}\t{rounds}\t{r['accuracy']:.4f}\t{r['mean_seconds']:.3f}\t{r['tokens_per_second']:.2f}\t{r['mean_calls']:.2f}")
