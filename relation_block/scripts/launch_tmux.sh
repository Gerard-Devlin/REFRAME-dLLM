...
set -euo pipefail
PHASE="${1:-}"
case "$PHASE" in setup|weights|subset|prepare|preflight|native|baseline|train|evaluate|compare|smoke|pilot|full) ;; *) echo 'phase: setup weights subset prepare preflight native baseline train evaluate compare smoke pilot full'; exit 2;; esac
...
if [[ "$PHASE" != setup && "$PHASE" != weights && "$PHASE" != subset && "$PHASE" != prepare && "$PHASE" != compare ]]; then
    ...
    if [[ "$PHASE" != train && "$PHASE" != smoke && "$PHASE" != pilot && "$PHASE" != full && "$GPU_IDS" == *,* ]]; then
...
for name in LR SAVE_EVERY EVAL_EVERY FINAL_EVAL_LIMIT ARM STEPS GLOBAL_BATCH MICRO_BATCH SEED MAX_SECONDS CHECKPOINT RESUME EVAL_LIMIT ROUNDS MAX_NEW_TOKENS EVAL_SPLIT COMPARE_RUNS CONDA_ROOT CONDA_ENV MIN_FREE_GIB SUBSET_DIR SUBSET_TOKENS SEQ_LENGTH REASONING TRAIN_SOURCE; do
