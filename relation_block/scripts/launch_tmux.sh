...
set -euo pipefail
PHASE="${1:-}"
case "$PHASE" in setup|weights|subset|prepare|preflight|native|baseline|train|evaluate|compare|smoke|pilot) ;; *) echo 'phase: setup weights subset prepare preflight native baseline train evaluate compare smoke pilot'; exit 2;; esac
...
for name in ARM STEPS GLOBAL_BATCH MICRO_BATCH SEED MAX_SECONDS CHECKPOINT RESUME EVAL_LIMIT ROUNDS MAX_NEW_TOKENS EVAL_SPLIT COMPARE_RUNS CONDA_ROOT CONDA_ENV MIN_FREE_GIB SUBSET_DIR SUBSET_TOKENS SEQ_LENGTH REASONING TRAIN_SOURCE; do
