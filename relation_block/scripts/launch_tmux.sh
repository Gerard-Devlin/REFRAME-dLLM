...
set -euo pipefail
PHASE="${1:-}"
case "$PHASE" in setup|weights|subset|prepare|preflight|native|baseline|train|evaluate|compare|smoke|pilot|full|diagnose-length|diagnose-zero|continuation|resident|boundary-ab|lora-sweep) ;; *) echo 'phase: setup weights subset prepare preflight native baseline train evaluate compare smoke pilot full diagnose-length diagnose-zero continuation resident boundary-ab lora-sweep'; exit 2;; esac
...
if [[ "$PHASE" != setup && "$PHASE" != weights && "$PHASE" != subset && "$PHASE" != prepare && "$PHASE" != compare ]]; then
    ...
    if [[ "$PHASE" != train && "$PHASE" != smoke && "$PHASE" != pilot && "$PHASE" != full && "$PHASE" != diagnose-zero && "$PHASE" != continuation && "$PHASE" != resident && "$PHASE" != boundary-ab && "$PHASE" != lora-sweep && "$GPU_IDS" == *,* ]]; then
...
if [[ "$PHASE" == evaluate && ! -d "${CHECKPOINT:-}" ]]; then echo 'Set CHECKPOINT to a full checkpoint or train directory'; exit 2; fi
...
for name in SOURCE_RUN RECONSTRUCTION_LIMIT LR SAVE_EVERY EVAL_EVERY FINAL_EVAL_LIMIT ARM STEPS GLOBAL_BATCH MICRO_BATCH SEED MAX_SECONDS CHECKPOINT RESUME EVAL_LIMIT ROUNDS MAX_NEW_TOKENS EVAL_SPLIT COMPARE_RUNS CONDA_ROOT CONDA_ENV MIN_FREE_GIB SUBSET_DIR SUBSET_TOKENS SEQ_LENGTH REASONING TRAIN_SOURCE MODEL_SIZES; do
