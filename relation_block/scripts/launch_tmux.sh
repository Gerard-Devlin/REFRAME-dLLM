...
if [[ "$PHASE" != setup && "$PHASE" != weights && "$PHASE" != subset && "$PHASE" != prepare && "$PHASE" != compare ]]; then
    ...
    if [[ "$PHASE" != train && "$PHASE" != smoke && "$PHASE" != pilot && "$PHASE" != full && "$PHASE" != diagnose-zero && "$PHASE" != continuation && "$PHASE" != resident && "$GPU_IDS" == *,* ]]; then
