...
def main():
    ...
    for item in source:
        ...
        if key in seen:
            ...
            continue
        seen.add(key)
        rows.append({"ids": full, "prefix": len(prompt)})
    random.Random(args.seed).shuffle(rows)
    if len(rows) < 1024:
        raise ValueError("Too few eligible examples")
    heldout, train = rows[:256], rows[256:]
    ...
    m = dict(version=1, model_id=MODEL_ID, revision=REVISION, source=str(args.source),
             source_sha256=digest(args.source), length=args.length, block_size=args.block_size,
             ...
             scope="Alpaca feasibility adaptation, not reproduction of v2 full training recipe; exact-only contamination check")
