...
for NAME in \
  ...
  RUN_DIR GPU_IDS DATASET NEMOTRON_ROOT PREPARED SUPERVISION SPLIT ATTEMPTS TRAIN_SIZE VALIDATION_SIZE SHARD_SIZE OVERSAMPLE \
  LIMIT UPDATES OVERFIT_RECORDS INIT_ADAPTER RESUME STEPS ADAPTER MERGED_MODEL CHECKPOINT; do
  if [[ -v "$NAME" ]]; then
    printf 'export %s=%q\n' "$NAME" "${!NAME}" >> "$ENV_FILE"
  fi
done
JOB_FILE="$RUN_DIR/job.sh"
WORKDIR=$(pwd)
cat > "$JOB_FILE" <<EOF
#!/usr/bin/env bash
set -o pipefail
...
cd $(printf '%q' "$WORKDIR")
bash llada_step_distill/scripts/run.sh $(printf '%q' "$MODE") 2>&1 | tee $(printf '%q' "$LOG")
code=\${PIPESTATUS[0]}
echo \$code > $(printf '%q' "$RUN_DIR/exit_code")
exit \$code
EOF
chmod +x "$JOB_FILE"
tmux new-session -d -s "$SESSION" "bash '$JOB_FILE'"
